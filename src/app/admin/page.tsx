...
export default function AdminDashboard() {
  ...
  const [content, setContent] = useState<SiteContent>({
    ...
    services: [],
    portfolio: [],
    theme: { primaryColor: "#0052FF", secondaryColor: "#FFC700" },
    navigation: [],
  ...
  const addNavLink = () => {
    setContent(prev => ({
      ...prev,
      navigation: [...(prev.navigation || []), { label: "Nouveau Lien", href: "/" }]
    }));
  };
  ...
  const addFeature = () => {
    setContent(prev => ({
      ...prev,
      features: [...(prev.features || []), { icon: "Zap", title: "Nouvel Atout", desc: "Description..." }]
    }));
  };
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {loading ? (
              ...
              <>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-2 transition-all ${
                        activeTab === tab.id 
                          ? "bg-afro-blue text-white shadow-md shadow-[#0052FF]/20" 
                ...
                {activeTab === "global" && (
                  <div className="glass-light p-8 rounded-3xl shadow-sm border-[#0052FF]/10 flex flex-col gap-6">
                    <h3 className="text-sm font-black uppercase tracking-wider text-zinc-950 flex items-center gap-2 border-b border-zinc-100 pb-4">
                      <Palette className="w-4 h-4 text-afro-blue" /> Paramètres Globaux (Thème & Animations)
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div className="flex flex-col gap-2">
                        <label className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Couleur Primaire (Hex)</label>
                        <div className="flex items-center gap-3">
                          <input type="color" value={content.theme?.primaryColor || "#0052FF"} onChange={e => setContent(prev => ({...prev, theme: {...prev.theme, primaryColor: e.target.value}}))} className="w-10 h-10 rounded-xl cursor-pointer" />
                          <input type="text" value={content.theme?.primaryColor || "#0052FF"} onChange={e => setContent(prev => ({...prev, theme: {...prev.theme, primaryColor: e.target.value}}))} className="flex-1 bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-2 text-xs font-mono uppercase" />
                        </div>
                      </div>
                      <div className="flex flex-col gap-2">
                        <label className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Couleur Secondaire (Hex)</label>
                        <div className="flex items-center gap-3">
                          <input type="color" value={content.theme?.secondaryColor || "#FFC700"} onChange={e => setContent(prev => ({...prev, theme: {...prev.theme, secondaryColor: e.target.value}}))} className="w-10 h-10 rounded-xl cursor-pointer" />
                          <input type="text" value={content.theme?.secondaryColor || "#FFC700"} onChange={e => setContent(prev => ({...prev, theme: {...prev.theme, secondaryColor: e.target.value}}))} className="flex-1 bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-2 text-xs font-mono uppercase" />
                        </div>
                      </div>
                      <div className="flex items-center justify-between bg-zinc-50 border border-zinc-200 p-4 rounded-xl md:col-span-2">
                        <div className="flex flex-col">
                          <span className="text-xs font-black uppercase text-zinc-950">Animations au défilement</span>
                          <span className="text-[10px] text-zinc-500">Activer les effets d'apparition (Framer Motion)</span>
                        </div>
                        <button onClick={() => setContent(prev => ({...prev, animations: { enabled: !(prev.animations?.enabled ?? true) }}))} className={`w-12 h-6 rounded-full relative transition-colors ${content.animations?.enabled !== false ? 'bg-afro-blue' : 'bg-zinc-300'}`}>
                          <span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${content.animations?.enabled !== false ? 'left-7' : 'left-1'}`} />
                        </button>
                      </div>
                    </div>
                  </div>
                ...
                {/* SECTION 4: NAVIGATION EDITING */}
                {activeTab === "nav" && (
                  <div className="glass-light p-8 rounded-3xl shadow-sm border-[#0052FF]/10 flex flex-col gap-6">
                    <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
                      <h3 className="text-sm font-black uppercase tracking-wider text-zinc-950 flex items-center gap-2">
                        <Navigation className="w-4 h-4 text-afro-blue" /> Menu de Navigation
                      </h3>
                      <button onClick={addNavLink} className="px-4 py-2 bg-gradient-to-r from-afro-blue/10 to-afro-blue/15 border border-[#0052FF]/15 text-afro-blue text-[10px] font-black uppercase tracking-wider rounded-full hover:bg-[#0052FF]/20 transition-all flex items-center gap-1.5">
                        <Plus className="w-3.5 h-3.5" /> Ajouter un lien
                      </button>
                    </div>
                    <div className="flex flex-col gap-4">
                      {content.navigation?.map((nav, idx) => (
                        <div key={idx} className="flex items-center gap-4 bg-zinc-50 border border-zinc-100 p-4 rounded-2xl relative group">
                          <div className="flex-1 flex flex-col gap-1.5">
                            <label className="text-[9px] font-black uppercase tracking-wider text-zinc-400">Libellé</label>
                            <input type="text" value={nav.label || ""} onChange={e => {
                                const next = [...(content.navigation || [])];
                                next[idx] = { ...nav, label: e.target.value };
                                setContent(prev => ({...prev, navigation: next}));
                              }} 
                              className="bg-white border border-zinc-200 rounded-xl px-4 py-2 text-xs text-zinc-950"
                            />
                          </div>
                          <div className="flex-1 flex flex-col gap-1.5">
                            <label className="text-[9px] font-black uppercase tracking-wider text-zinc-400">Lien (URL ou #ID)</label>
                            <input type="text" value={nav.href || ""} onChange={e => {
                                const next = [...(content.navigation || [])];
                                next[idx] = { ...nav, href: e.target.value };
                                setContent(prev => ({...prev, navigation: next}));
                              }} 
                              className="bg-white border border-zinc-200 rounded-xl px-4 py-2 text-xs text-zinc-950"
                            />
                          </div>
                          <button onClick={() => removeNavLink(idx)} className="p-2 mt-4 bg-rose-50 border border-rose-100 rounded-xl hover:bg-rose-100 text-rose-500">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                      ))}
                ...
                {activeTab === "features" && (
                  <div className="flex flex-col gap-8">
                    ...
                    <div className="glass-light p-8 rounded-3xl shadow-sm border-[#0052FF]/10 flex flex-col gap-6">
                      <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
                        <h3 className="text-sm font-black uppercase tracking-wider text-zinc-950 flex items-center gap-2">
                          <Zap className="w-4 h-4 text-afro-blue" /> Atouts (Features Grid)
                        </h3>
                        <button onClick={addFeature} className="px-4 py-2 bg-gradient-to-r from-afro-blue/10 to-afro-blue/15 border border-[#0052FF]/15 text-afro-blue text-[10px] font-black uppercase tracking-wider rounded-full hover:bg-[#0052FF]/20 transition-all flex items-center gap-1.5">
                          <Plus className="w-3.5 h-3.5" /> Ajouter
                        </button>
                      </div>
                      <div className="flex flex-col gap-4">
                        {content.features?.map((feat, idx) => (
                          <div key={idx} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-zinc-50 border border-zinc-100 p-6 rounded-2xl relative group">
                            <button onClick={() => removeFeature(idx)} className="absolute top-4 right-4 p-2 bg-rose-50 border border-rose-100 rounded-xl hover:bg-rose-100 text-rose-500 opacity-0 group-hover:opacity-100">
                              <Trash2 className="w-4 h-4" />
                            </button>
                            <div className="flex flex-col gap-1.5">
                              <label className="text-[9px] font-black uppercase tracking-wider text-zinc-400">Icône (Nom Lucide, ex: Zap)</label>
                              <input type="text" value={feat.icon || ""} onChange={e => {
                                  const next = [...(content.features || [])];
                                  next[idx] = { ...feat, icon: e.target.value };
                                  setContent(prev => ({...prev, features: next}));
                                }} 
                                className="bg-white border border-zinc-200 rounded-xl px-4 py-2 text-xs text-zinc-950"
                              />
                            </div>
                            <div className="flex flex-col gap-1.5">
                              <label className="text-[9px] font-black uppercase tracking-wider text-zinc-400">Titre</label>
                              <input type="text" value={feat.title || ""} onChange={e => {
                                  const next = [...(content.features || [])];
                                  next[idx] = { ...feat, title: e.target.value };
                                  setContent(prev => ({...prev, features: next}));
                                }} 
                                className="bg-white border border-zinc-200 rounded-xl px-4 py-2 text-xs text-zinc-950 font-bold"
                              />
                            </div>
                            <div className="flex flex-col gap-1.5 md:col-span-3">
                              ...
                              <textarea rows={2} value={feat.desc || ""} onChange={e => {
                                  const next = [...(content.features || [])];
                                  next[idx] = { ...feat, desc: e.target.value };
                                  setContent(prev => ({...prev, features: next}));
                                }} 
                                className="bg-white border border-zinc-200 rounded-xl px-4 py-2 text-xs text-zinc-950 resize-none"
                              />
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="glass-light p-8 rounded-3xl shadow-sm border-[#FFC700]/15 flex flex-col gap-6">
                      <div className="flex items-center justify-between border-b border-zinc-100 pb-4">
                        <h3 className="text-sm font-black uppercase tracking-wider text-zinc-950 flex items-center gap-2">
                          <Rocket className="w-4 h-4 text-afro-gold" /> Appel à l'action Final (CTA)
                        </h3>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Titre (Badge)</label>
                          <input type="text" value={content.cta?.title || ""} onChange={e => setContent(prev => ({...prev, cta: {...prev.cta, title: e.target.value}}))} className="bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-xs text-zinc-950" />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Texte du Bouton</label>
                          <input type="text" value={content.cta?.buttonLabel || ""} onChange={e => setContent(prev => ({...prev, cta: {...prev.cta, buttonLabel: e.target.value}}))} className="bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-xs text-zinc-950" />
                        </div>
                        <div className="flex flex-col gap-2 md:col-span-2">
